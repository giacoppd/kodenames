...
class bighandler(Factory):
    ...
    def __init__(self, table, mx, heavy): #TODO make less huge
        ...
        n = int((mx - 2)/2) #the number of players on the red or blue teams that aren't spys
        ...
        if(heavy == 0): #which team goes first
            for y in range(0,n):
                self.order.append(rorder[y])
                self.order.append(border[y])
        else:
            for z in range(0,n):
                self.order.append(border[z])
                self.order.append(rorder[z])
        if(heavy == 0):
